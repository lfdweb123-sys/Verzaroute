...
function StatCard({ icon: Icon, label, value, href }: { icon: typeof Wallet; label: string; value: string; href?: string }) {
  const content = (
    <div className="rounded-2xl border border-white/10 bg-obsidian-card p-6 card-glow transition-all h-full">
    ...
    <>
      ...
      <div className="p-6 md:p-8 space-y-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          ...
          <StatCard icon={TrendingUp} label="Crédits consommés (récent)" value={totalCreditsUsed.toLocaleString("fr-FR")} />
        </div>
            ...
            <div className="space-y-2">
              {recentLogs.map((log) => (
                <div key={log.id} className="flex items-center justify-between text-sm border-b border-white/5 pb-2 last:border-0">
                  <span className="text-white/80">{log.model}</span>
